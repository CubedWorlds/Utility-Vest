    ...
    public void discover(FMLModContainer container) {
            ...
            for (ModFileScanData.AnnotationData annotation : data.getAnnotations()) {
                if (annotation.annotationType().equals(Type.getType(Config.class))) {
                    try {
                        ...
                        UtilityVest.LOGGER.error("Failed to load config class {}", annotation.clazz().getClassName(), e);
                    }
                }
            }
        }
        configs.forEach((type, list) -> {
            list.forEach(clazz -> {
                ...
                container.registerConfig(type, spec, UtilityVest.MOD_ID + "/" + UtilityVest.MOD_ID + "-" + type.extension() + ".toml");
            });
        });
    }
            ...
            if (cache.spec == spec) {
                cache.values.forEach((field, value) -> {
                    try {
                        ...
                        UtilityVest.LOGGER.error("Failed to update config field {}", field.getName(), e);
                    }
                });
            }
        });
    ...
    protected void generate(Class<?> clazz, Cache cache, ModConfigSpec.Builder builder) {
        ...
        for (Field field : clazz.getDeclaredFields()) {
            ...
            try {
                ...
                UtilityVest.LOGGER.error("Failed to access field {}", field.getName(), e);
            } catch (NullPointerException e) {
                UtilityVest.LOGGER.error("Config Field value {} is null or not static", field.getName(), e);
            }
        }
    }
